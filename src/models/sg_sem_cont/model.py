...
class MappingNetwork(nn.Module):
    def __init__(self, latent_dim=16, style_dim=64, num_domains=2, nr=5):
        super().__init__()
        self.nr = nr
        layers = []
...
class StyleEncoder(nn.Module):
    def __init__(self, img_size=256, style_dim=64, num_domains=2, max_conv_dim=512, nr=5, n_unshared_layers=0):
        super().__init__()
...
def build_model(args):
    ...
    mapping_network = MappingNetwork(args.latent_dim, args.style_dim, args.num_domains, nr=args.repr_dim)
    style_encoder = StyleEncoder(args.img_size, args.style_dim, args.num_domains, nr=args.repr_dim)
...
def semantics(sem_type, sem_path):
    if sem_type == 'moco' and sem_path is not None:
        return ss_model(sem_path)
    elif sem_type == 'vgg':
        dim = 29
        vgg = vgg19(pretrained=True).features[:dim]
        vgg.eval()
        return vgg
    else:
        print('Invalid choice of continuous semantics')
